...
from e2efold.evaluation import all_test_only_e2e, model_eval_all_test
...
if __name__ == '__main__':
    ...
    if LOAD_MODEL and os.path.isfile(model_path):
        ...
        contact_net.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
    if LOAD_MODEL and os.path.isfile(pp_model_path):
        print('Loading pp model...')
        lag_pp_net.load_state_dict(torch.load(pp_model_path, map_location=torch.device('cpu')))
    ...
    def per_family_evaluation():
        ...
        for rna_type in e2e_result_df['type'].unique():
            ...
            to_output = list(map(str,
                                 list(df_temp[['exact_p', 'exact_r', 'exact_f1', 'shift_p', 'shift_r',
                                               'shift_f1']].mean().values.round(3))))
            print(to_output)
        ...
        for epoch in range(epoches_third):
            ...
            for contacts, seq_embeddings, matrix_reps, seq_lens in train_generator:
                ...
                if steps_done % OUT_STEP == 0:
                    ...
                    loss_plot = [[loss_plot], [epoch, loss_u, loss_a, loss]]
    ...
    result_dict = all_test_only_e2e(test_generator, contact_net, lag_pp_net, device, test_data)
np.save('../results/loss_plot.npy', loss_plot)